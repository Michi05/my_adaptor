...
kindOfProperty = set(["dynParam", "readOnlyParam", "publishedTopic", "topic", "service", "virtual", "renaming"])
...
##TODO: It may be better to assume that the name is a topic if the "interpret" fails instead of the other way around
        if manager3D.is_published(topicName):
            return topicName
        else:
            rospy.logdebug(topicName + " -- doesn't exist as a path. Assuming that it is a name.")
...
# (Watch out; I don't really trust this yet; needs testing)
    ...
    def retransmitTopic(self, times, source_topic, output_topic, data_type = UInt16):
        ...
        if is_published(source_topic) == False:
...
#==================================================
    @staticmethod
    def is_published(topicName):
        for i in rospy.get_published_topics('/'):
            if topicName in i:
                return True
        print topicName + " topic doesn't exist"
        return False
